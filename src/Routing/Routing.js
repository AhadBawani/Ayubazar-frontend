...
import { getDiscountOfferHandler, getProductByIdRequestHandler } from '../RequestHandlers/RequestHandler/ProductRequestHandler';
...
const Routing = () => {
    ...
    useEffect(() => {
        if (coupon && usercart.length > 0) {
            let sum = 0;
            usercart?.forEach((item) => {
                sum += parseInt(Object.values(item?.option)) * parseInt(item?.quantity);
            })
            let discountAmount = Math.floor((sum * coupon?.percentage) / 100);
            dispatch(DiscountAction(discountAmount));
        }
    }, [coupon, usercart, dispatch])
    ...
    useEffect(() => {
        if (user && user?.shippingAddress) {
            if (user?.shippingAddress?.city.toLowerCase() === 'rajkot') {
                const obj = {
    ...
    return (
        <>
            <BrowserRouter>
                ...
                <div className={mobile ? 'max-w-sm mt-24' : 'max-w-lg mt-36'}>
                    <Routes>
                ...
                </div>
                {
                    ...
                    &&
                    <div className={mobile ? 'max-w-sm h-full' : 'max-w-lg h-full'}>
                        <CommonFooter />
                    </div>
                }
                {
