...
const UserCartCard = ({ item }) => {
    ...
    const handleRemoveProduct = (product, option) => {
        setIsProcessing(true);
        if (!user) {
            const arr = [...localUserCart];
            const index = arr.findIndex((item) => item?.id === product?._id
    ...
    return (
        <>
            <div className='mb-6 relative'>
                ...
                <div className='flex'>
                    <div className='min-h-[90px] max-h-[90px] min-w-[90px] max-w-[90px] inline-block align-middle' style={{ border: '1px solid #ececec' }}>
                        <img className='max-w-[100%]' src={Requests.GET_PRODUCT_IMAGE + item?.product?.productImage} alt='Ayubazar' />
                    </div>
                    <div className='ml-2 flex flex-col flex-grow'>
                        <div style={{ maxWidth: '240px' }} className="whitespace-normal">
                            <span className='transition-all ease-in-out duration-300 text-[#333] text-base hover:text-[#d0bdac] font-semibold cursor-pointer' style={{ lineHeight: '1.2' }}>
                                {item?.product?.productName}
                            </span>
                        </div>
                        <div className='flex mt-auto'>
                            <div className='flex justify-center h-[20px] items-center p-2 cursor-pointer transition-all duration-200 ease-in-out hover:text-[#d0bdac] hover:bg-[#f1f1f1]' style={{ border: '1px solid #ddd', borderTopLeftRadius: '8px', borderBottomLeftRadius: '8px' }} onClick={handleSubtractQuantity}>
                            ...
                            <div className='flex ml-2'>
                                ...
                                <div className='flex ml-2 mt-[-2px]'>
                                    ...
                                    <span>{Object.values(item?.option)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className='absolute bottom-0 right-0'>
                    <div className='flex justify-center items-center'>
                        <div className='flex'>
                            <LuIndianRupee className='mt-1' />
                            <span>{parseInt(item?.quantity) * parseInt(Object.values(item?.option))}</span>
                        </div>
                    </div>
                </div>
            </div>
