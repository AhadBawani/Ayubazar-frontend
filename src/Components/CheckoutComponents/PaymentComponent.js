...
import { CodChargesAction, OrderDetailsAction, TotalAction } from '../../Redux/Actions/UserActions/UsersAction';
...
const PaymentComponent = () => {
    const { orderDetails, shipping, total, codCharges } = useUserState();
    const userId = localStorage.getItem('userId');
    const [isProcessing, setIsProcessing] = useState(false);
    const [selectedPaymentMethod, setSelectedPaymentMethod] = useState('PhonePe');
    const codAmount = Math.round(0.05 * total);
            ...
            <div className='mt-6'>
                ...
                <div className='border border-[#ddd] rounded-lg p-4 mt-3'>
                    <label className='flex items-center space-x-2 cursor-pointer'>
                        <input type='radio' value='PhonePe' checked={selectedPaymentMethod === 'PhonePe'} onChange={handlePaymentMethodChange} />
                    ...
                    <div className='mx-4 my-6'>
                        <span style={{ lineHeight: '1.4', fontSize: '100%' }}>Pay securely by UPI/Phone Pe/ Credit or Debit cards/ Internet Banking/ Other Wallets through Paytm Secure Servers.</span>
                    </div>
                    <label className='flex items-center space-x-2 cursor-pointer'>
                        <input type='radio' value='COD' checked={selectedPaymentMethod === 'COD'} onChange={handlePaymentMethodChange} />
                        <span className='text-xl' style={{ color: '#4d4d4d', lineHeight: '1.6', fontWeight: 'bold' }}>Cash on Delivery</span>
                    </label>
                    <div className='mx-4 my-6'>
                        <span style={{ lineHeight: '1.4', fontSize: '100%' }}>In COD 5% will be charged on total bill amount {codAmount}/-</span>
                    </div>
                </div>
                <hr className='mt-4' />
                <div className='p-4'>
