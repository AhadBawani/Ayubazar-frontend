...
import { DiscountAction } from '../../Redux/Actions/UserActions/UsersAction';
import { LuIndianRupee } from 'react-icons/lu';
import { useDispatch } from 'react-redux';
import UserCartCard from './UserCartCard';

const OrderDetail = () => {
    const { usercart, total, discount, shipping, subTotal, couponDisable } = useUserState();
                ...
                .then((response) => {
                    if (response) {
                        if (response?.message === 'Invalid Coupon!') {
                            ...
                            let discount = (total * response?.percentage) / 100;
                            setTimeout(() => {
        ...
        <div style={{
            ...
            <div>
                {usercart?.map((item, index) => {
                    return <><UserCartCard item={item} index={index} /></>
                })}
            </div>
            {
                !couponDisable
