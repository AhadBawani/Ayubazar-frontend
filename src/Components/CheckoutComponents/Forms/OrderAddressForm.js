...
import { ShippingAction, ValidateFormAction } from '../../../Redux/Actions/UserActions/UsersAction';
import { useDispatch } from 'react-redux';
import { validateEmail } from '../../../Utils/verfiyEmail';
import { addBillingAddressHandler, addShippingAddressHandler, getUserByEmailHandler, getUserCartByIdHandler, getUserIfExistHandler, placeOrderHandler } from '../../../RequestHandlers/RequestHandler/UserRequestHandler';
import { useNavigate } from 'react-router-dom';

const OrderAddressForm = ({ userBillingAddress, userShippingAddress, setIsProcessing }) => {
    const { user, shipping, subTotal, usercart, validateForm, total } = useUserState();
    ...
    useEffect(() => {
        if (validateForm?.validate) {
            handleContinueShopping()
                .then((orderDetails) => {
                    if (orderDetails) {
                        const { codAmount, codCharges, handlingCharges, paymentMethod } = validateForm.data;
