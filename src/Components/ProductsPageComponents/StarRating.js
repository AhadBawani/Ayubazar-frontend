...
const StarRating = ({ starValue, showProgressBar, clickable = true, setStarValue, percentage }) => {
        ...
        <div className="flex items-center">
            {[1, 2, 3, 4, 5].map((star) => (
                <svg
                    ...
                    className={`h-4 w-4 fill-current cursor-pointer ${star > 1 ? 'ml-1' : ''}`}
                    viewBox="0 0 24 24"
                ...
                >
                    <path
                        fill={(starValue >= star || hoverValue >= star) ? '#FFD700' : '#DCDCDC'}
                        stroke={(starValue >= star || hoverValue >= star) ? '#FFD700' : '#DCDCDC'}
                        strokeLinecap="round"
