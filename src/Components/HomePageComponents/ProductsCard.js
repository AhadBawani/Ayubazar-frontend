...
const ProductsCard = ({ product, dispatch }) => {
    ...
    const productOptions = product?.options === null ? [] : product?.isVariationAvailable && JSON.parse(product.options);
    ...
    useEffect(() => {
        if (product?.isVariationAvailable && showOptions.length === 0) {
            let arr = [];
    ...
    useEffect(() => {
        if (selectedOption && product?.discount > 0) {
            const originalDiscountOption = JSON.parse(product?.originalOptions)?.
                filter((item) => Object.values(item)[0] === Object.keys(selectedOption)[0]);

            if (originalDiscountOption?.length > 0) {
                setDiscountPrice(originalDiscountOption[0]);
            }
        }
    ...
    const handleAddToCart = (e) => {
        ...
        if (product?.isVariationAvailable && !selectedOption) {
            if (mobile) {
                setIsHovered(true);
            };            
        } else {
        ...
        <div
            ...
            >
                <div className='p-2 flex justify-between'>
                    ...
                    {
                        isHovered && product?.isVariationAvailable && !product?.outOfStock
                        &&
                        <>
                            <OptionSelection
            ...
            <div className='w-[190px] mt-2'>
                                                        ...
                                                        <>
                                                            <div className='flex'>
                                                                <div className='text-[#777777]'>
                                                                    <del className='flex'>
                                                                        <LuIndianRupee className='mt-1' /> {discountPrice?.price}.00
                                                                    </del>
                                                                </div>
                                                                <div className='flex ml-2'>
                                                                    <LuIndianRupee className='mt-1' />
                                                                    {Object.values(selectedOption)[0]}.00
                                                                </div>
                                                            </div>
                                                        </>
                                                        :
                ...
                <div className='w-[190px] mt-2'>
                    <div className='mt-2 sm:ml-4 mb-4'>
                        {((mobile || isHovered) && !product?.outOfStock) &&
                            <>
                                {
                                    mobile
                                        ?
